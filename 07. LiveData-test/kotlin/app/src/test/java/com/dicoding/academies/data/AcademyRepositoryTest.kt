...
class AcademyRepositoryTest {
    ...
    fun getAllModulesByCourse() {
        ...
        `when`(local.getModulesByCourse(courseId)).thenReturn(dummyModules)

        val courseEntities =
            LiveDataTestUtil.getValue(academyRepository.getAllModulesByCourse(courseId))
        verify(local).getModulesByCourse(courseId)
        assertNotNull(courseEntities.data)
    ...
    fun getCourseWithModules() {
        ...
        dummyEntity.value =
            DataDummy.generateDummyCoursesWithModules(DataDummy.generateDummyCourses()[0], false)
